...
class DataIterator(object):
    def __init__(self, tune_lens, tune_idxs, batch_size, random_lens=False):
        ...
        for k, v in zip(tune_lens, tune_idxs): #change izip --> zip
    ...
    def __iter_homogeneous_lens(self):
        for idxs in self.len2idx.values(): #Change itervalues() --> values()
            self.rng.shuffle(idxs)

        progress = defaultdict(int)
        available_lengths = list(self.len2idx.keys()) #Change self.len2idx.keys() --> list(self.len2idx.keys())
