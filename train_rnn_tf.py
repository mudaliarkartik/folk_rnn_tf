...
_, train_op, loss_op, var_lr = build_model(x, mask, seq_length, prob_dropout, config, vocab_size)
...
with tf.Session(config=conf) as sess:
    ...
    for epoch in range(start_epoch, config.max_epoch):
        for train_batch_idxs in train_data_iterator:
            ...
            _, train_loss = sess.run([train_op, loss_op], feed_dict={x: x_batch, mask: mask_batch, seq_length: sequence_length, prob_dropout: config.dropout})
            ...
            if niter % config.validate_every == 0:
                ...
                for valid_batch_idx in valid_data_iterator:
                    ...
                    avg_valid_loss += sess.run(loss_op, feed_dict = {x: x_batch, mask: mask_batch, seq_length: sequence_length, prob_dropout: 0.0})
